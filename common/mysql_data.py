...
def get_data_source_id_list(*relate_id):
    ...
    for item in result:
        data_source_id_list.append(item[0])
    data_source_id_list.append(2)
    return data_source_id_list


def get_data_source(company_id):
    data_source_id_list = []
    query_set_list = SemMonitorCrawlInfo.objects.filter(company_id=int(company_id))
    for item in query_set_list:
        data_source_id_list.append(item.company_name)
    return data_source_id_list

if __name__ == "__main__":
    # print get_data_source_id_list(1)
    # get_data_source(1)
    get_company_id(1)
